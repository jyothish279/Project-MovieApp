...
describe('login page', () => {
  ...
  it('should get username input box', () => {
    ...
    .toBeTruthy(`<input id="username" formControlName='username'> should exist in login.component.html`);
  });
  ...
  it('should login into the system', () => {
    ...
    page.getCurrentURL().then((url) => {
      if (url.indexOf('login') > -1) {
        ...
        page.clickSubmitButton();
        page.navigateToDashboard();
        expect(page.getCurrentURL()).toContain('login', 'Should navigate to dashboard');
      } else {
        expect(page.getCurrentURL()).toContain('login', 'Should navigate to dashboard');
      }
    });
